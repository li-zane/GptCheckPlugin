...
export function upstreamDiscoveryCopy(rateWritesEnabled: boolean): UpstreamDiscoveryCopy {
  if (rateWritesEnabled) {
    return {
      ...
      upstreamAriaPrefix: "探测并应用上游",
      upstreamTitle: "探测余额、分组与目标倍率，并应用账号计费倍率",
      allSuccess: "全部上游探测并应用完成；账号状态与倍率日志已刷新。",
      allError: "批量探测并应用失败",
      empty: "暂无可探测并应用的上游",
    };
  }
  return {
    bulkLabel: "探测全部上游",
    bulkBusyLabel: "探测中",
    upstreamAriaPrefix: "探测上游",
    upstreamTitle: "探测余额、分组与目标倍率",
    allSuccess: "全部上游探测完成；余额、分组与目标倍率已刷新，未修改账号计费倍率。",
    allError: "批量探测失败",
    empty: "暂无可探测的上游",
  };
}

export function upstreamDiscoverySuccessMessage(rateWritesEnabled: boolean, upstreamName: string) {
  return rateWritesEnabled
    ? `已完成「${upstreamName}」探测并应用；账号状态与倍率日志已刷新。`
    : `已探测「${upstreamName}」的余额、分组与目标倍率；未修改账号计费倍率。`;
}

export function upstreamDiscoveryErrorMessage(rateWritesEnabled: boolean, upstreamName: string) {
  return `${upstreamName} ${rateWritesEnabled ? "探测并应用" : "探测"}失败`;
}
...
export function apiAccountSyncMessage(
  ...
  if (result.total === 0) return "未在管理站点中发现可同步的 API 账号。";
  if (result.probe_globally_enabled === false) {
    return `API 账号清单同步完成：全局上游探测已关闭，已跳过 ${result.total} 个上游。`;
  }
  const skipped = result.skipped || 0;
  const cached = result.cached || 0;
  const cachedSuffix = cached ? `，${cached} 个上游使用缓存` : "";
  if (result.failed > 0) {
    return `API 账号同步完成：${result.succeeded}/${result.total} 个上游${action}成功，${result.failed} 个失败${cachedSuffix}${skipped ? `，${skipped} 个按上游设置跳过` : ""}。`;
  }
  return `API 账号同步完成：${result.succeeded} 个上游${action}成功${cachedSuffix}${skipped ? `，${skipped} 个按上游设置跳过` : ""}。`;
}
